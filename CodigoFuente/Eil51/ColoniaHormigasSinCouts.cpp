...
class ColoniaHormigas {
	...
	public:
		...
		void ParametrosIniciales() {
			...
			cout<<"Desea que la ciudad inicial sea aleatoria o desea empezar en alguna (0-Iniciar en posicion a indicar | 1-Iniciar en punto aleatorio)"<<endl;
			bool aleatoria=false;
			cin>>aleatoria;
			if(aleatoria) {
				cout<<"El punto de inicio sera aleatorio"<<endl;
				puntoInicial=rand()%50;
				puntoInicialtipo=true;
				//solucion aleatoria
			} else {
				//
				cout<<"Dame la ciudad del punto de inicio"<<endl;
				puntoInicialtipo=false;
				do {
					cin>>puntoInicial;
				} while(puntoInicial<=0||puntoInicial>=51);
				...
				for(int hormiga=0; hormiga<cantidadHormigasIteracion; hormiga++) {
					...
					do {
						...
						for(int i=0; i<ciudades; i++) {
							...
							if(contaNullos>=50||(auxiliarSumaProbabilidad==0&&i==50)) {
